 ...
 */
export async function fetchArticleById(articleId: number): Promise<ApiArticle | null> {
  try {
    // 从环境变量或配置中获取API基础URL
    ...
    if (result.success && result.data) {
      ...
      if (result.data.publishedDate) {
        result.data.publishedDate = formatDateForFrontend(result.data.publishedDate);
      }
      if (result.data.updatedDate) {
        result.data.updatedDate = formatDateForFrontend(result.data.updatedDate);
      }
      
      // 处理标签和分类
      if (result.data.tags && typeof result.data.tags === 'string') {
        result.data.tags = result.data.tags.split(',').map(tag => tag.trim()).filter(tag => tag.length > 0);
      }
 ...
 * @returns Promise<ApiArticle | null>
 */
export async function createArticle(articleData: Partial<ApiArticle>): Promise<ApiArticle | null> {
  try {
    const apiBaseUrl = import.meta.env.PUBLIC_API_BASE || '/api';
    ...
    const dataToSend = {
      ...
      // 转换分类格式
      category: articleData.categories && articleData.categories.length > 0 ? articleData.categories[0] : undefined
    };
    
    const response = await fetch(`${apiBaseUrl}/api/admin/articles`, {
      method: 'POST',
      headers: {
 ...
 * @returns Promise<ApiArticle | null>
 */
export async function updateArticle(articleId: number, articleData: Partial<ApiArticle>): Promise<ApiArticle | null> {
  try {
    const apiBaseUrl = import.meta.env.PUBLIC_API_BASE || '/api';
    ...
    const response = await fetch(`${apiBaseUrl}/api/admin/articles/${articleId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
 ...
 */
export function transformApiArticleToFrontend(apiArticle: ApiArticle): FrontendArticle {
  // 基础字段映射
